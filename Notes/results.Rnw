...
<<label=tablex3, echo=FALSE, results='asis'>>=
setwd("~/Documents/github/pheno_bc")
spwithbb <- read.csv("output/bb.success.ax.trt.csv", head = TRUE)
library(xtable)

mytable <- data.frame(spwithbb, row.names = "Species")
names(mytable) <- c("Proportion Budburst")
make.mytable <- xtable(mytable, caption="Proporation of samples with budburst per species")
