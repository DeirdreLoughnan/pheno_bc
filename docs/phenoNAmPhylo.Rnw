...
% currently 232
Species life history events are advancing with climate change. But these phenological impacts of climate change are spatially implicit, and found to have variable effects across species. Understanding the causes of this variability is critical if we are to accurately predict future changes and phenology's cascading impacts on communities. We took a more holistic approach to account for the diversity of drives that shape phenology. Using spring budburst as our study system, we tested the relative importance of population-level variation, species differences, and the potentially confounding effects of phylogenetic structure on phenology across variable temperature and photoperiod cues. We combined budburst data from two growth chamber studies and used a Bayesian phylogenetic mixed effects model to estimate cue responses of 47 woody plant species from four populations across eastern and western North America. While we found weak population-level differences in budburst and cues, this phenological event had a strong phylogenetic structure. Cue responses, however, only explained \Sexpr{diffBAEastHigh} to \Sexpr{diffBAWest}\% of variation. The remaining species-level variation suggests there are additional traits driving budburst phenology in woody plant communities. Our findings suggest that current phenology is largely the result of environmental cues, but evolutionary history and not geographic differences also plays an imporant role. Understanding the unidentified drivers of species-level differences, whether they be physiological constraints or other key environmental cues, is critical for understanding species phenology and future responses under our changing climate. 
