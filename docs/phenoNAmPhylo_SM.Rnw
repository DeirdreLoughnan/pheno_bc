...
\begin{table}[ht]
\centering
\caption{Approximate chill units from our two western sites, E.C. Manning Park and Smithers B.C., Canada, and our two eastern sites, Harvard Forest, USA and St. Hippolyte, Canada. Weather data was obtained from the Hope Slide weather station for our E.C. Manning Park estimates and the Smithers airport weather station for our Smithers communities. For our eastern communities, weather data was obtained from weather stations at Harvard Forest and in St. Hippolyte.} 
\label{tabChill}
\begin{tabular}{llrrr}
  \hline
Population & Chilling treatment & Chill hours & Utah model & Chill portions \\ 
  \hline
...
% chill$Chilling.treatment<- gsub("degree ","$\\circ$", chill$Chilling.treatment)
% names(chill) <- c("Population","Chilling treatment","Chill hours","Utah model","Chill portions")
% 
...
\begin{figure}[h!]
    \centering
     ...
     \caption{Estimated day of budburst in response to (a) forcing across populations under low chilling and short photoperiods, (b) chilling across populations under low forcing and short photoperiods, and (c) across photoperiods under our baseline forcing and chilling for species sampled from our four populations. The thin error bars represent the 90\% uncertainty interval, while the thicker error bars represent the 50\% uncertainty interval, and symbols the mean.} 
    \label{fig:intrxn3}
\end{figure}
\newpage
...
\begin{figure}[h!]
    ...
    \caption{Comparisons of posterior distributions for cues estimates between shrub and tree species. Black circles represent the median cues, while the thinner black line the 90\% quantile interval. The coloured distribution is the the posterior density of cues responses for all species within a given architectural type. The y-axis spans the entire range of the data.}
    \label{fig:cueST}
    \end{figure}
...
\begin{figure}[h!]
    \centering
     ...
     \caption{Estimated changes in species ranked budburst order, compared between species level effects (species intercept) and under high cues (a and d), species level effects and under low cues (b and e), and ranked order under low and high cues (c and f) for our eastern species in red (a-c) and western species in blue (d-f). For the three species that occur in both transect, shown in yellow, \emph{Alnus incana} exhibited the greatest rank change with a difference of three, while \emph{Betula papyrifera} had a rank difference of two and \emph{Populus tremuloides} a rank difference of one.}
    \label{fig:rank}
\end{figure}
...
% \begin{figure}[h!]
%     \centering
%      \includegraphics[width=\textwidth]{..//figures/bothRanked.pdf}
%      \caption{The difference in budburst ranking between high and low cues for the three species that occurr in both the eastern (shown in green) and western transect (shown in purple). } %emwOct25: Not so much info in this figure; maybe better to mention the high and low changes possible in caption to previous figures: Changes varied from a high of XX rank diff (Alnus incana) to a low of ...
%     \label{fig:bothRanked}
% \end{figure}

\begin{figure}[h!]
    \centering
     \includegraphics[width=\textwidth]{..//figures/phyloIntColor.pdf}
     \caption{Species differences were accounted for by including phylogenetic effects on the species intercept in a model estimating days to budburst after the start of forcing treatments. We pruned to our species subset an existing phylogeny for flowering plants developed by Smith and Brown (2018)} %emwOct25 -- add units to intercept (days) on figure.
    \label{fig:phylo}
\end{figure}

\begin{table}[ht]
\centering
\caption{Mean budburst dates across all treatments from raw data for 47 species at our two western sites, E.C. Manning Park and Smithers B.C., Canada, and our two eastern sites, Harvard Forest, USA and St. Hippolyte, Canada.} 
\begin{tabular}{lrrrr}
  \hline
...
% 
% meanBBTab <- xtable(mdlTab, caption="Mean budburst dates across all treatments from raw data for 47 species at our two western sites, E.C. Manning Park (MP) and Smithers B.C.(SM) Canada, and our two eastern sites, Harvard Forest (HF) USA and St. Hippolyte (SH) Canada.")
% print(meanBBTab,include.rownames=F, caption.placement="top", hline.after=c(-1,0), sanitize.text.function = identity,digits=c(0,0,0,0,0))
% 
% @
% 
...
make.bb_suc <- xtable(bb_suc, caption="Proportion of surviving samples per species for which budburst was observed in our western controlled enviornment study. See Flynn and Wolkovich (2018) for survival in our eastern study.")
