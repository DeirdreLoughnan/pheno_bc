...
\begin {enumerate}
\item Species cue responses were strongly phylogenetically structured (with a lambda of \Sexpr{lamT} (uncertainty interval: \Sexpr{lamTLower}, \Sexpr{lamTUpper}), with a root trait value of \Sexpr{rootT} (uncertainty interval: \Sexpr{rootTLower}, \Sexpr{rootTUpper}).
...
\item Forcing and chilling cues produced a strong delaying effect, with low chilling being offset by hight forcing conditions(Fig. \ref{fig:intrxn} a). 
\item Similar delaying interactions occurr between forcing and the two eastern sites and between chilling and the two eastern sites. 
...
\item Our more southern site in our western transect, Manning Park, showed delayed budburst realtive to our more northern Smithers sites. 
...
\item While there are some differences across sites, they are moderate.

\item Individual species show more distinct trends - compare the three species in both transects

\item No strong differnces across plant architypes
...
\begin{figure}[h!]
    \centering
     ...
     \caption{Interaction plots of day of budburst of first bud in response a) chill portions and forcing, b) forcing cues for species sampled from St. Hollypte, c) chilling cues for species sampled from St. Hollypte, and d) photoperiod cues and species sampled from St. Hollypte. }
    \label{fig:intrxn}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Posterior distributions of estimated cue responses with site level effects for individual sites, depicting a) chilling, b) forcing, and c) photoperiod cue responses. Black circles represent the median shift in phenology for a phenological event, while the thinner black line the 90\% quantile interval. The coloured distribution is the the posterior density of the posteriors of the cue responses and site level responses for all species at a given site. The y-axis spans the entire range of the data.}
    \label{fig:siteCues}
\end{figure}

\begin{figure}[h!]
    \centering
     \includegraphics[width=\textwidth]{..//figures/siteCueInter.pdf} 
    \caption{Posterior distributions of estimated cue responses with site level effects and site interacti ons for individual sites, depicting a) chilling, b) forcing, and c) photoperiod cue responses. Black circles represent the median shift in phenology for a phenological event, while the thinner black line the 90\% quantile interval. The coloured distribution is the the posterior density of the posteriors of the cue responses and site level responses for all species at a given site. The y-axis spans the entire range of the data.}
    \label{fig:siteCueInter}
\end{figure}
