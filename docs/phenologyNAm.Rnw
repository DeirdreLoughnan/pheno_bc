...
\begin{figure}[h!]
    ...
    \caption{Posterior distributions of estimated cue responses with site level effects and site interactions for individual sites, depicting a) chilling, b) forcing, and c) photoperiod cue responses. Black circles represent the median shift in phenology for a phenological event, while the thinner black line the 90\% quantile interval. The coloured distribution is the the posterior density of the posteriors of the cue responses and site level responses for all species at a given site. The y-axis spans the entire range of the data.}
    \label{fig:cues}
\end{figure}
