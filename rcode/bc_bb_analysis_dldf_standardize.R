...
# To properly add site to the model I am going to use the indexing approach used on pg 153 of statistical rethinking, modeling code was drafted by Lizzie - nope, I am not sure how to add interactions to that, so I am going to focus on dummy variables

# modified Feb 8, 2022
# summarizing what models I have run
rm(list=ls())
...
# 
# 
# mdl.t <- stan("stan/bc_bb_ncpphoto_ncpinter_standardize_dummy.stan",
#               data = datalist_z,
#               iter = 4000, chains=1)
# #######################################################################
# 
...
# 
# shrubs.both = c("VIBLAN","RHAFRA","RHOPRI","SPIALB","VACMYR","VIBCAS", "AROMEL","ILEMUC", "KALANG", "LONCAN", "LYOLIG", "alninc","alnvir","amealn", "corsto","loninv", "menfer","rhoalb", "riblac","rubpar","samrac","shecan","sorsco","spibet","spipyr","symalb","vacmem","vibedu")
...
# X <- model.matrix(tbb ~ chillport + force.2z + photo.2z, data = pheno.t)
# 
# datalist_z <- list( N=nrow(pheno.t),
#                     Nsite = nrow(pheno.t),
#                     n_sp = length(unique(pheno.t$species.fact)),
...
#                     X= X)
# 
# mdl.i <- stan("stan/dummy_var_stackoverflow.stan",
#               data = datalist_z,
#               iter = 4000, chains=1)
# 
# load("output/tbb_cport_stnd_dummy.Rds")
# 
# sumi <- summary(mdl.d)$summary
# 
# site <- sumi[grep("a_site", rownames(sumi)), "mean"]; site
# force <- sumi[grep("mu_force", rownames(sumi)), "mean"]; force
# 
# col4fig <- c("mean","sd","25%","50%","75%","Rhat")
# mu_params <- c("grand_mu",
#                 "mu_force",
#                "mu_photo",
#                "mu_chill",
#                # "mu_inter_fp",
