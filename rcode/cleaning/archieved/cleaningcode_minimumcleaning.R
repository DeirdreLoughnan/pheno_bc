...
gc <- subset(d, day <= 84)
head(d)
gc$bbch.t <- as.numeric(as.character(gc$bbch.t))
gc$bbch.l <- as.numeric(as.character(gc$bbch.l))
gc$bbch2.l <- as.numeric(as.character(gc$bbch2.l))
gc$bbch3.l <- as.numeric(as.character(gc$bbch3.l))
gc$percent.t <- as.numeric(as.character(gc$percent.t))
gc$percent.l <- as.numeric(as.character(gc$percent.l))
gc$percent2.l <- as.numeric(as.character(gc$percent2.l))
gc$percent3.l <- as.numeric(as.character(gc$percent3.l))
...
head(pheno)
#write.csv(pheno, "input/day.of.bb.miniclean.csv", row.names = FALSE)

# Compared to the fully cleaned data, this only results in 5 differnces, mostly for latbb80, I am confident that my cleaning is not wrong and ok to do.
