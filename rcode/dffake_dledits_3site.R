...
mdl.full <- stan("stan/df_interdirect.stan",
                 ...
                 include = FALSE, pars = c("ypred_new","y_hat"),
                 iter = 4000, chains= 4, warmup = 2000)
save(mdl.full, file = "output/df_interdirect.Rda")
...
slopeFC <- as.data.frame(sum[grep("b_inter_wc", rownames(sum)), ]) 
slopeCP <- as.data.frame(sum[grep("b_inter_cp", rownames(sum)), ]) 
slopeFP <- as.data.frame(sum[grep("b_inter_wp", rownames(sum)), ]) ; slopeFP <- slopeFP[c(9:16),]


par(mfrow = c(4,2))
plot( rnorm(nsp, warmdiff, warmdiff.sd) ~ slopeF$mean, pch = 19)
