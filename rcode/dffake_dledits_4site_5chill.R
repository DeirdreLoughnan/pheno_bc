...
# March 23: my first go at adding 4 sites (with no ncp) had 485 divergent transitions - my interpretation is that the interactions with photo and forcing could do with the ncp
...
mm <- model.matrix(~(site2 + site3 + site4 + warm + photo + chillP)^2, data.frame(site2, site3, site4, warm, photo, chillP))
...
for(i in 1:nsp){ # loop over species, as these are the random effect modeled
  ...
  fakex <- data.frame(bb, sp = i, site2, site3,site4, warm, photo, chillP)
...
names(values) <- c("int","b.site2","b.site3","b.site4","b.force", "b.photo","b.chill1","b.fs2","b.ps2","b.cs2","b.fs3","b.ps3","b.cs3","b.fs4","b.ps4","b.cs4","b.fp","b.fc", "b.pc")
...
# mdl.full <- stan("stan/df_4sites.stan",
#                  data = datalist,
#                  include = FALSE, pars = c("ypred_new","y_hat"),
#                  iter = 4000, chains= 4, warmup = 3000)
# save(mdl.full, file = "output/df_4site_noyrep_March23.Rda")

mdl.full <- stan("stan/df_noncp_4sites.stan",
                 data = datalist,
                 include = FALSE, pars = c("ypred_new","y_hat"),
                 iter = 4000, chains= 4, warmup = 3000)
save(mdl.full, file = "output/df_4site_noncp_March23.Rda")
...
#load("output/df_4site_noyrep_March23.Rda")
...
sum <- summary(mdl.full)$summary
 summary(mdl.full)$summary[c("mu_a", "mu_b_warm","mu_b_chill1","mu_b_photo","b_site2","b_site3","b_site4",
                             "mu_b_inter_wp","mu_b_inter_wc1","mu_b_inter_pc1",
                             "mu_b_inter_ps2","mu_b_inter_ps3","mu_b_inter_ps4","mu_b_inter_s2c1","mu_b_inter_s3c1","mu_b_inter_s4c1","mu_b_inter_ws2","mu_b_inter_ws3","mu_b_inter_ws4",
                             "sigma_b_warm","sigma_b_chill1","sigma_b_photo","sigma_a",
                             "sigma_b_inter_wp","sigma_b_inter_wc1","sigma_b_inter_pc1",
                             "sigma_b_inter_ps2","sigma_b_inter_ws3","sigma_b_inter_ws4","sigma_b_inter_ws2","sigma_b_inter_ps3","sigma_b_inter_ps4","sigma_b_inter_s2c1","sigma_b_inter_s3c1","sigma_b_inter_s4c1", "sigma_y"),c("mean","25%","75%")]
...
slopeFP <- as.data.frame(sum[grep("b_inter_wp", rownames(sum)), ]) ; slopeFP <- slopeFP[c(2:36),]
slopeFC <- as.data.frame(sum[grep("b_inter_wc1", rownames(sum)), ]) ; slopeFC <- slopeFC[c(2:36),]
slopeCP <- as.data.frame(sum[grep("b_inter_pc1", rownames(sum)), ]) ; slopeCP <- slopeCP[c(2:36),]
slopeFS2 <- as.data.frame(sum[grep("b_inter_ws2", rownames(sum)), ]) ; slopeFS2 <- slopeFS2[c(2:36),]
slopePS2 <- as.data.frame(sum[grep("b_inter_ps2", rownames(sum)), ]) ; slopePS2 <- slopePS2[c(2:36),]
slopeCS2 <- as.data.frame(sum[grep("b_inter_s2c1", rownames(sum)), ]) ; slopeCS2 <- slopeCS2[c(2:36),]
slopeFS3 <- as.data.frame(sum[grep("b_inter_ws3", rownames(sum)), ]) ; slopeFS3 <- slopeFS3[c(2:36),]
slopePS3 <- as.data.frame(sum[grep("b_inter_ps3", rownames(sum)), ]) ; slopePS3 <- slopePS3[c(2:36),]
slopeCS3 <- as.data.frame(sum[grep("b_inter_s3c1", rownames(sum)), ]) ; slopeCS3 <- slopeCS3[c(2:36),]

slopeFS4 <- as.data.frame(sum[grep("b_inter_ws4", rownames(sum)), ]) ; slopeFS4 <- slopeFS4[c(2:36),]
slopePS4 <- as.data.frame(sum[grep("b_inter_ps4", rownames(sum)), ]) ; slopePS4 <- slopePS4[c(2:36),]
slopeCS4 <- as.data.frame(sum[grep("b_inter_s4c1", rownames(sum)), ]) ; slopeCS4 <- slopeCS4[c(2:36),]


slopeSite2 <- as.data.frame(sum[grep("b_site2", rownames(sum)), ]) ; mean(values$b.site2)
slopeSite3 <- as.data.frame(sum[grep("b_site3", rownames(sum)), ]) ; mean(values$b.site3)
slopeSite4 <- as.data.frame(sum[grep("b_site4", rownames(sum)), ]) ; mean(values$b.site4)


pdf("fake_esti_correlations_35spp_4site_5chill.pdf", height = 10, width = 15)
par(mfrow = c(4,4))
...
slopeFC <- as.data.frame(sum[grep("b_inter_wc1", rownames(sum)), ]) ; slopeFC <- slopeFC[c(1:35),]
slopeCP <- as.data.frame(sum[grep("b_inter_pc1", rownames(sum)), ]) ; slopeCP <- slopeCP[c(2:36),]
slopeFS2 <- as.data.frame(sum[grep("b_inter_ws2", rownames(sum)), ]) ; slopeFS2 <- slopeFS2[c(38:72),]
slopePS2 <- as.data.frame(sum[grep("b_inter_ps2", rownames(sum)), ]) ; slopePS2 <- slopePS2[c(38:72),]
slopeCS2 <- as.data.frame(sum[grep("b_inter_s2c1", rownames(sum)), ]) ; slopeCS2 <- slopeCS2[c(2:36),]
slopeFS3 <- as.data.frame(sum[grep("b_inter_ws3", rownames(sum)), ]) ; slopeFS3 <- slopeFS3[c(38:72),]
slopePS3 <- as.data.frame(sum[grep("b_inter_ps3", rownames(sum)), ]) ; slopePS3 <- slopePS3[c(38:72),]
slopeCS3 <- as.data.frame(sum[grep("b_inter_s3c1", rownames(sum)), ]) ; slopeCS3 <- slopeCS3[c(2:36),]

slopeFS4 <- as.data.frame(sum[grep("b_inter_ws4", rownames(sum)), ]) ; slopeFS4 <- slopeFS4[c(38:72),]
slopePS4 <- as.data.frame(sum[grep("b_inter_ps4", rownames(sum)), ]) ; slopePS4 <- slopePS4[c(38:72),]
slopeCS4 <- as.data.frame(sum[grep("b_inter_s4c1", rownames(sum)), ]) ; slopeCS4 <- slopeCS4[c(2:36),]


slopeSite2 <- as.data.frame(sum[grep("b_site2", rownames(sum)), ]) ;slopeSite2; mean(values$b.site2)
slopeSite3 <- as.data.frame(sum[grep("b_site3", rownames(sum)), ]) ;slopeSite3; mean(values$b.site3)
slopeSite4 <- as.data.frame(sum[grep("b_site4", rownames(sum)), ]) ;slopeSite4; mean(values$b.site4)
