...
nsp = 30 #number of species - Its really hard to estimate a random effect with only 5 groups 
nind = 20 
...
fake$bb <-  mu_grand + alpha.site[1] + fake$alpha.pheno.sp + fake$alpha.force.sp *  fake$warm + 
    ...
    fake$alpha.site*fake$site + fake$alpha.fc * (fake$chill*fake$warm) +
  #fake$alpha.fp * (fake$photo*fake$warm)+ fake$alpha.cp * (fake$photo*fake$chill) + 
    fake$gen.var
...
summary(lm(bb ~  warm + photo + chill + site + warm*chill 
           #+ warm*photo + photo*chill
           , data = fake)) # 
...
# The values are very close between trail data and lmer output
  ...
  mdl.simpdum <- stan("stan/test_model_interactions_truepriors.stan",
                  data = datalist,
                  include = FALSE, pars = c("ypred_new","y_hat"),
                  iter = 4000, chains= 4, warmup = 2000)

  #save(mdl.simpdum, file = "output/simBBPosterior.Ddata")
  save(mdl.simpdum, file = "output/fj_int_30spp_march18.Rda")

load("output/fj_int_march17.Rda")

  sum <- summary(mdl.simpdum)$summary
  sum[c("mu_grand","mu_force","mu_chill","mu_photo","mu_fc","mu_cp","mu_fp","b_site","sigma_a","sigma_force","sigma_chill","sigma_photo","sigma_fc","sigma_cp","sigma_fp","sigma_y"),c(1,5,7)]
  ...
  slopeFP <- as.data.frame(sum[grep("b_fp", rownames(sum)), ]) 
  
  pdf("test_fj_int.pdf")
  par(mfrow = c(4,2))
  plot(alpha.pheno.sp ~ intSp$mean, pch = 19);abline(1,1)
  plot(alpha.force.sp ~ slopeF$mean, pch = 19);abline(1,1)
  plot(alpha.chill.sp ~ slopeC$mean, pch = 19);abline(1,1)
  plot(alpha.photo.sp ~ slopeP$mean, pch = 19);abline(1,1)
  plot(alpha.fc ~ slopeFC$mean, pch = 19);abline(1,1)
  plot(alpha.cp ~ slopeCP$mean, pch = 19);abline(1,1)
  plot(alpha.fp ~ slopeFP$mean, pch = 19);abline(1,1)
  dev.off()
