...
/*	mu_b_inter_wp ~ normal(0, 35); // Delete because all in NCP now
  ...
  sigma_interceptsa ~ normal(1,15);//(1,15)
...
/*	b_inter_wp ~ normal(mu_b_inter_wp, sigma_b_inter_wp); // Delete because all in NCP now
	...
	b_photo_ncp ~ normal(0, 50); 
	b_warm_ncp ~ normal(0, 50); 
	
  b_inter_wp_ncp ~ normal(0, 50); 
  b_inter_ws_ncp ~ normal(0, 50);
	b_inter_ps_ncp ~ normal(0, 50);
	b_inter_wc1_ncp ~ normal(0, 50);		
	b_inter_pc1_ncp ~ normal(0, 50);		
	b_inter_sc1_ncp ~ normal(0, 50);		
