...
transformed parameters { // Vectorize: Won't save time probably here (no scalar x vector)
  ...
  // b_inter_sc2 = mu_b_inter_sc2 + sigma_b_inter_sc2*b_inter_sc2_ncp;*/
	...
	for(i in 1:N){
		...
		b_inter_wp[sp[i]] * (warm[i] * photo[i]) +
		b_inter_ws[sp[i]] * (warm[i] * site[i]) +
		b_inter_ps[sp[i]] * (site[i] * photo[i]) +
		b_inter_wc1[sp[i]] * (warm[i] * chill1[i]) +
		// b_inter_wc2[sp[i]] * inter_wc2[i] +
		b_inter_pc1[sp[i]] * (chill1[i] * photo[i]) +
		// b_inter_pc2[sp[i]] * inter_pc2[i] +
		b_inter_sc1[sp[i]] * (chill1[i] * site[i]) 
