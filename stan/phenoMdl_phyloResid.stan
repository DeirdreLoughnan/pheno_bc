// Started May 4, 2026 to address reviewer comments---asking for PGLS like model with phylogeny included on the residuals
...
transformed parameters { 
  ...
  b_inter_ps4 = mu_b_inter_ps4 + sigma_b_inter_ps4*b_inter_ps4_ncp;
	...
	for(i in 1:N){
		y_hat[i] = a_sp[sp[i]] +	e_sp[i] +
    b_site2 * site2[i] + b_site3 * site3[i] +  b_site4 * site4[i] +
		b_warm[sp[i]] * warm[i] +  b_photo[sp[i]] * photo[i] +  b_chill1[sp[i]] * chill1[i]  
		+ b_inter_wp[sp[i]] * inter_wp[i] + b_inter_wc1[sp[i]] * inter_wc1[i] +
		b_inter_pc1[sp[i]] * inter_pc1[i] + b_inter_s2c1[sp[i]] * inter_s2c1[i] +
		b_inter_ws2[sp[i]] * inter_ws2[i] +	b_inter_ps2[sp[i]] * inter_ps2[i] +
		b_inter_s3c1[sp[i]] * inter_s3c1[i] +	b_inter_ws3[sp[i]] * inter_ws3[i] +
		b_inter_ps3[sp[i]] * inter_ps3[i] +	b_inter_s4c1[sp[i]] * inter_s4c1[i] 
		+ b_inter_ws4[sp[i]] * inter_ws4[i] + b_inter_ps4[sp[i]] * inter_ps4[i]
		;
		}
...
model {
  ...
  sigma_a ~ normal(0,10);
	...
	e_sp ~ multi_normal(rep_vector(e_z,n_sp), lambda_vcv(Vphy, lam_interceptsa, sigma_interceptsa)); 
	e_z ~ normal(0,30); //(5,10)
