...
model {
	...
	mu_a ~ normal(30,30); //(5,10)
	sigma_a ~ normal(0,10);
	
	a_resid ~ multi_normal(rep_vector(sigma_y,n_sp), lambda_vcv(Vphy, lam_interceptsa, sigma_interceptsa)); 

	lam_interceptsa ~ beta(1,1);
  sigma_interceptsa ~ normal(1,100); //(1,10)
